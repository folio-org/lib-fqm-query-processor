...
public class DerivedTableIdentificationService {
  ...
  public DerivedTableIdentificationService(MetaDataRepository metaDataRepository) {
  ...
  public String getDerivedTable(String tenantId, EntityType mainEntity, Fql fql, boolean shouldSortResults) {
    ...
    String candidateEntityTypeId = subEntityTypeIds.stream()
      .map(subEntityTypeId -> metaDataRepository.getEntityTypeDefinition(tenantId, subEntityTypeId))
      ...
      .orElse(mainEntity)
      .getId();
    String derivedTableName = metaDataRepository.getDerivedTableName(tenantId, UUID.fromString(candidateEntityTypeId))
