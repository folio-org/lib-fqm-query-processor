...
public class FqlToSqlConverter {
  ...
  private final SqlFieldIdentificationService sqlFieldIdentificationService;

  public FqlToSqlConverter() {
    this(new FqlService(), new SqlFieldIdentificationService());
  }

  FqlToSqlConverter(FqlService fqlService, SqlFieldIdentificationService sqlFieldIdentificationService) {
    this.fqlService = fqlService;
    this.sqlFieldIdentificationService = sqlFieldIdentificationService;
  }
  ...
  private Field<Object> field(FieldCondition<?> condition, EntityType entityType) {
    ...
    return entityType.getColumns()
      ...
      .findFirst()
      .map(sqlFieldIdentificationService::getSqlField)
